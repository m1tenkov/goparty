...
4. Для `message_new` и `message_event` payload кладется во внутреннюю очередь через `enqueue_callback_payload(...)`.
5. Если очередь заполнена, endpoint возвращает `503 Callback queue is full`.
6. Если событие принято в очередь, VK сразу получает ответ `ok`.
7. Worker, запущенный при старте приложения через `start_callback_worker()`, забирает payload из очереди.
8. В `event_processing.py` создается объект события и вызывается прикладной обработчик.
9. Ошибки внутри логики не роняют endpoint: они логируются в `event_processing.py`.
...
- Callback endpoint не выполняет пользовательский сценарий синхронно: он кладет событие в очередь, чтобы быстро ответить VK.
