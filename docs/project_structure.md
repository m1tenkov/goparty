...
Слой доступа к БД. Отвечает за runtime-миграции схемы, профили, игры, сведения о фотографиях, лайки, дизлайки, вычисление мэтчей, фильтры, историю и пользовательские сессии.
...
Очистка таблицы `interactions` без удаления анкет, игр, фильтров и фотографий. Отдельных таблиц `matches` и `pending_likes` в актуальной схеме нет: мэтчи и входящие лайки вычисляются по встречным записям в `interactions`.
...
7. `database.py` сохраняет анкеты, фильтры, пути к фотографиям, взаимодействия и сессии; мэтчи определяются по двум встречным лайкам.
